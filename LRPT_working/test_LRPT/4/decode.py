...
def decode_image(input_binary_path, output_image_path, flag_length=250, access_code_interval=1024):
    with open(input_binary_path, 'rb') as f:
        ...
        # Define the custom string
        custom_string = b'11100001010110101110100010010011'
        
        # Find the first instance of the custom string
        first_index = data.find(custom_string)
        if first_index == -1:
            raise ValueError("First custom string not found!")
        
        # Find the second instance of the custom string
        second_index = data.find(custom_string, first_index + len(custom_string))
        if second_index == -1:
            raise ValueError("Second custom string not found!")
        
        # Start reading after the second instance of the custom string
        start_index = second_index + len(custom_string)
        
        # Find the start flag (250 zero bytes)
        #zero_flag = b'\xDD' * flag_length
        #start_index = data.find(zero_flag, start_index)
        #if start_index == -1:
